...
# # Xử lý một file JSON
# python OCR/postprocess_ocr_v3.py --input runs/ocr/10103.ocr.json --output runs/ocr_post/10103.post.ocr.json

# # Xử lý toàn bộ thư mục
# python OCR/postprocess_ocr_v3.py --input runs/ocr --output runs/ocr_post
...
# Định nghĩa các class IDs hợp lệ cho v3 pipeline
# 0: start, 1: end, 2: inputoutput, 3: operation, 4: subroutine, 5: condition
V3_NODE_CLASS_IDS = {0, 1, 2, 3, 4, 5}
...
def _clean_spaces(text: str) -> str:
    """Xóa khoảng trắng thừa và normalize"""
    text = text.replace("\n", " ")
...
def _repair_code_layout(text: str) -> str:
    """Sửa lỗi OCR trong code layout"""
    out = _clean_spaces(text)
 
    # Merge identifiers split across OCR lines
    for _ in range(3):
        updated = out
        
        # Fix multiple underscores thành single underscore
        updated = re.sub(r"_{2,}", "_", updated)
        
        # Merge split identifiers with underscores/dots
        updated = re.sub(
        ...
        # Fix input/output formatting - THÊM KHOẢNG TRẮNG sau dấu :
...
def _fix_quotes(text: str) -> str:
    """Fix lỗi quotes trong OCR"""
    # Fix double quotes bị nhận nhầm
    text = re.sub(r':\s*"', r": '", text)  # :" -> :'
    text = re.sub(r'"\s*\)', r"')", text)  # ") -> ')
    
    # Fix các pattern thường gặp
    text = re.sub(r'\(\s*:\s*"', r"(':'", text)  # (:" -> (':'
    text = re.sub(r'"\s*if\s+', r"' if ", text)  # "if -> 'if
    text = re.sub(r'\s+else\s+"', r" else '", text)  # else " -> else '
    
    return text
 
 
def normalize_text(text: str, class_name: str) -> str:
    """Normalize text dựa trên class"""
    out = _repair_code_layout(text)
    
    # Fix quotes cho tất cả các class
    out = _fix_quotes(out)
    
    # Class-specific normalization
    if class_name in ["inputoutput", "operation"]:
        # Đảm bảo có khoảng trắng sau "input:" và "output:"
        out = re.sub(r'\b(input|output):', r'\1: ', out, flags=re.IGNORECASE)
...
def _box_to_rect(box: list[list[float]]) -> tuple[float, float, float, float]:
    """Convert box coordinates to rectangle"""
...
def _ordered_text_from_lines(ocr_lines: list[dict[str, Any]], min_conf: float) -> str:
    """Sắp xếp text từ OCR lines theo vị trí"""
    words = []
    for ln in ocr_lines:
...
def postprocess_payload(payload: dict[str, Any], min_conf: float) -> dict[str, Any]:
    ...
    for node in nodes:
        ...
        # XỬ LÝ CHO CÁC NODE KHÔNG THUỘC 6 CLASSES
        if class_id not in V3_NODE_CLASS_IDS:
            # Vẫn thử xử lý OCR nếu có dữ liệu
            if ocr_lines:
                ...
                node["normalized_text"] = _clean_spaces(ordered_text)
            else:
                # Nếu không có OCR data thì để trống
                node["ordered_text"] = ""
                node["normalized_text"] = ""
            
            # Thêm warning flag
            node["postprocess_warning"] = f"Node class_id={class_id} not in V3_NODE_CLASS_IDS"
            continue
 
        # XỬ LÝ CHO CÁC NODE HỢP LỆ (6 CLASSES)
        ordered_text = _ordered_text_from_lines(ocr_lines, min_conf=min_conf)
        if not ordered_text:
            ordered_text = _clean_spaces(str(node.get("ocr_text", "")))
