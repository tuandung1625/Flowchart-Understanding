# Evaluation script for full-image OCR results
# Đánh giá kết quả OCR toàn bộ ảnh so với ground truth

# python OCR/evaluate_ocr_full.py \
#   --ocr-dir runs/ocr_full_batch \
#   --flowchart-dir DATASET/Train/flowchart \
#   --output runs/ocr_eval/train_summary_full.json \
#   --match class \
#   --ignore-case
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate full-image OCR JSON outputs against .flowchart ground truth."
    )
    parser.add_argument(
        "--ocr-dir",
        type=str,
        required=True,
        help="Directory containing full-image OCR JSON outputs",
    )
    parser.add_argument(
        "--flowchart-dir",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="runs/ocr_eval/summary_full.json",
...
def parse_ocr_nodes_full(ocr_json_path: Path) -> list[dict[str, Any]]:
    """Parse node_texts from full-image OCR JSON"""
...
def pair_nodes_order(
    gt_nodes: list[dict[str, str]], pred_nodes: list[dict[str, Any]]
) -> list[tuple[dict[str, str], dict[str, Any] | None]]:
    pairs: list[tuple[dict[str, str], dict[str, Any] | None]] = []
...
def evaluate_one(
    ...
    pred_nodes = parse_ocr_nodes_full(ocr_json_path)
