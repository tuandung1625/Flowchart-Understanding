...
V3_NODE_CLASS_NAMES = {"start", "end", "inputoutput", "operation", "subroutine", "condition"}
V3_NODE_CLASS_IDS = {0, 1, 2, 3, 4, 5}

# python OCR/evaluate_ocr_v3.py --ocr-dir runs/ocr_post --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary.json --match class --ignore-case
# python OCR/evaluate_ocr_v3.py --ocr-dir runs/ocr_post --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary_strict.json --match class
# python OCR/evaluate_ocr_v3.py --ocr-dir runs/ocr_post --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary_order.json --match order --ignore-case
...
def parse_flowchart_nodes(flowchart_path: Path) -> list[dict[str, str]]:
    ...
    for raw in flowchart_path.read_text(encoding="utf-8").splitlines():
        ...
        if node_class not in V3_NODE_CLASS_NAMES:
            continue
...
def parse_ocr_nodes(ocr_json_path: Path, pred_field: str) -> list[dict[str, str]]:
    ...
    for idx, node in enumerate(nodes):
        ...
        is_v3_class = class_id in V3_NODE_CLASS_IDS or class_name in V3_NODE_CLASS_NAMES
        if not is_v3_class:
            continue
