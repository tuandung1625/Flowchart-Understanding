# Tác dụng: Xử lý OCR cho một hình ảnh duy nhất
    ...
    # Sử dụng YOLO để phát hiện các node (hộp, hình tròn, v.v.)
    # Sử dụng PaddleOCR để nhận diện text trong từng node
    # Xuất kết quả thành JSON file

# python OCR/ocr_nodes.py <đường_dẫn_ảnh> \
...
# EX:
# python OCR/ocr_nodes.py DATASET/Test/images/10103.png --model runs/segment/runs/flowchart_seg_v2_exp1/weights/best.pt --output runs/ocr 
...
def is_text_node_class(class_name: str) -> bool:
    return class_name not in ARROW_CLASS_NAMES
...
def main() -> None:
    ...
    for rank, i in enumerate(order, start=1):
        ...
        if is_text_node_class(class_name):
