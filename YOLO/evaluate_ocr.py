...
# python YOLO/evaluate_ocr.py --ocr-dir runs/ocr --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary.json --match class --ignore-case
# python YOLO/evaluate_ocr.py --ocr-dir runs/ocr --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary_strict.json --match class
# python YOLO/evaluate_ocr.py --ocr-dir runs/ocr --flowchart-dir DATASET/Train/flowchart --output runs/ocr_eval/train_summary_order.json --match order --ignore-case
FLOWCHART_NODE_RE = re.compile(r"^(\w+)=>(\w+):\s*(.*)$")
...
def parse_flowchart_nodes(flowchart_path: Path) -> list[dict[str, str]]:
    ...
    for raw in flowchart_path.read_text(encoding="utf-8").splitlines():
        ...
        if not line:
            continue
        m = FLOWCHART_NODE_RE.match(line)
        if not m:
            continue
        node_id, node_class, node_text = m.group(1), m.group(2), m.group(3)
        nodes.append({"id": node_id, "class_name": node_class, "text": node_text})
    return nodes


def parse_ocr_nodes(ocr_json_path: Path) -> list[dict[str, str]]:
...
def evaluate_one(
    ...
    pred_nodes = parse_ocr_nodes(ocr_json_path)
